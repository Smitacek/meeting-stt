...
export const LiveRecordingSDK: React.FC<LiveRecordingSDKProps> = ({
  ...
  const setupAzureSpeech = useCallback(async (tokenData: any) => {
    try {
      ...
      console.log('Setting up Azure Speech SDK with region:', tokenData.region);
      ...
      // Configure for Czech language and real-time
      ...
      recognizer.recognized = (_, e) => {
        if (e.result.reason === SpeechSDK.ResultReason.RecognizedSpeech && e.result.text.trim()) {
          ...
          const result: TranscriptionResult = {
            speaker: "Mluvčí 1", // Basic speaker for now
            text: e.result.text,
      ...
      recognizer.sessionStarted = (_, __) => {
  ...
  const startRecording = useCallback(async () => {
    try {
      ...
      console.log('Starting Azure Speech SDK recording...');
      ...
      // Setup Azure Speech SDK
      const recognizer = await setupAzureSpeech(tokenData);
    ...
    <Card className="w-full">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          Live Transcription (Azure Speech SDK)
        </CardTitle>
        <p className="text-sm text-muted-foreground">
          Direct Azure Speech Service connection • Real-time WebSocket streaming
        </p>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        {!isRecording && transcriptionResults.length === 0 && (
          <div className="text-center py-8 text-muted-foreground">
            ...
            <p className="text-sm">
              Klikněte na "Spustit nahrávání" pro začátek Azure Speech SDK transkripce
            </p>
            <p className="text-xs mt-2">
              Přímé WebSocket spojení s Azure Speech Service • Real-time streaming
            </p>
          </div>
        )}
      </CardContent>
